...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if curl -fsSL "$CONFIG_URL" -o "$TEMP_CONFIG"; then
        if [ -f "/etc/nxtmon/config.yaml" ]; then
            echo -e "${YELLOW}Existing configuration found. Creating backup at /etc/nxtmon/config.yaml.bak${NC}"
            sudo cp "/etc/nxtmon/config.yaml" "/etc/nxtmon/config.yaml.bak"
        fi
        sudo mv "$TEMP_CONFIG" /etc/nxtmon/config.yaml
        echo -e "Copied default.yaml to /etc/nxtmon/config.yaml"
    else
        echo -e "${RED}Failed to download default.yaml. You will need to manually create /etc/nxtmon/config.yaml${NC}"
    fi
